...
from FOD.Fusion import NDFusion
...
class FocusOnDepth(nn.Module):
    def __init__(self,
        ...
        for s in reassemble_s:
            self.fusions.append(NDFusion(resample_dim))
    ...
    def forward(self, img):
        ...
        for i in np.arange(0,len_fusion):
            ...
            reassemble_list.append(self.reassembles[i](activation_result))

        previous_stage=None
        for i in np.arange(len_fusion-1,-1,-1):
            fusion_result = self.fusions[i](reassemble_list[i], previous_stage, i)
