...
class FocusOnDepth(nn.Module):
    def __init__(self,
        ...
        self.model = model
        if self.model == "tf" or self.model == "dual":
            self.transformer_encoders = timm.create_model(model_timm, pretrained=True)
        elif self.model == "res" or self.model == "dual":
            #self.resnet = ResNet(resnet_type,resnet_src)
    ...
    def forward(self, img):
        ...
        if self.model == "res" or self.model == "dual":
            res_list = self.resnet(img)

        if self.model == "dual":
            # fusion 함수 작성
            for i in tf_reassemble_list:
                print(i.shape)
            for i in res_list:
                print(i.shape)
            pass
